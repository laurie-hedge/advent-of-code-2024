defmodule Solution do
	...
	def trail_score(_, 9, _) do 1 end
	def trail_score({x, y}, height, map) do
		steps = [{x + 1, y}, {x - 1, y}, {x, y + 1}, {x, y - 1}]
		valid_steps = Enum.filter(steps, fn step -> Map.get(map, step) == height + 1 end)
		scores = for step <- valid_steps, do: trail_score(step, height + 1, map)
		Enum.reduce(scores, 0, &Kernel.+/2)
	end

	def trail_score(start_pos, map) do
		trail_score(start_pos, 0, map)
	end
